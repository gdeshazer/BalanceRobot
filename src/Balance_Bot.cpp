...
private:
  ...
  float Q_angle  = 0.04; //0.001
  float Q_bias   = 0.001; //0.003 //0.035
  float R_measure  = 0.07; //0.03 //0.01
...
void loop(){
  float tilt = imu.findAdjustedTilt(0.1);  //0.1 time
...
//  Serial.print(kval[2]*100); Serial.print(", ");
  ...
  if(!imu.tooFar()){
    double pwm = pid.calculate(0,tilt, 1); //0.01
